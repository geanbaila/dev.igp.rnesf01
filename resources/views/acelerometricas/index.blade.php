...
@section('title', 'ESTACIONES DE ACELERÓMETRO DIGITAL')

@section('button')
    @can('create', \App\Business\Estacion\Acelerometrica\EstacionAcelerometrica::class)
        <div class="pull-right margin-bottom-10" style="letter-spacing: normal">
            <a class="btn blue-chambray rounded-1" href="{{ route('estaciones.acelerometricas.create') }}">
...
@section('content')
    ...
    <div class="pull-right">
        ...
        <form action="{{ route('estaciones.acelerometricas.previous') }}" method="post" style="display: none" id="items-form">
            {{ csrf_field() }}
            <input type="hidden" name="selected_items" id="selected-items" value="">
        </form>
    </div>
@if(count($prg_acelerometricas) > 0)
@foreach($prg_acelerometricas as $acelerometricas)
    @if($acelerometricas->count() > 0)
                <span id="total-items">{{ $acelerometricas->count() }} {{ str_plural('REGISTRO',$acelerometricas->count()) }}</span>
    @endif
    <table class="table table-striped table-bordered table-hover dt-responsive dttable" cellspacing="0" width="100%">
        @if($acelerometricas->count() > 0)
            ...
            <tr style="font-weight:bolder; color:#555555;background-color: #F1F1F1;">
                ...
                <td class="all text-center">ETHERNET</td>
                <td class="all text-center">CONF. WEB</td>
                <td class="all text-center">FUENTE</td>
                <td class="all">N° DÍAS ENERGÍA</td>
        ...
        <tbody>
        @foreach($acelerometricas as $keys => $station)
            ...
            <tr>
                ...
                <td>{{ $station->marca }}</td>
                <td>{{ $station->modelo }}</td>
                <td>{{ $station->frec_muestreo }}</td>
                <td>{{ $station->cap_almacenamiento }}</td>
                <td class="text-center">
                    @if($station->ethernet === 1)
                ...
                <td class="text-center">
                    {{ $station->fuente_220vac }}
                </td>
                <td>{{ $station->dias_respaldo }}</td>
                <td class="text-center" style="min-width: 142px !important;">
                    <div class="btn-group pull-right">
                        <a href="{{ route('estaciones.acelerometricas.edit', [ $encrypted_id]) }}" class="btn btn-xs btn-default"
                        ...
                        <ul class="dropdown-menu pull-right">
                            <li>
                                <a href="{{ route('estaciones.acelerometricas.edit', [ $encrypted_id]) }}"><i class="fa fa-edit"></i> Ver/Editar</a>
                            </li>
                            ...
                            @can('delete', $station)
                                ...
                                <li>
                                    ...
                                    <form id="del-station-{{$station->id}}" method="POST" class="hidden" action="{{ route('estaciones.acelerometricas.destroy',$encrypted_id) }}">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
